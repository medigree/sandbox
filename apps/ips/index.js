document.addEventListener("DOMContentLoaded", () => {
    ...
    function populateHeader(bundle) {
        ...
        if (!compositionEntry) {
            console.error('No Composition resource found in the bundle');
            return;
        }
        const composition = compositionEntry.resource;

        const patientEntry = bundle.entry.find(entry => entry.fullUrl.endsWith(composition.subject.reference.split('/')[1]));
        if (!patientEntry) {
            console.error('No Patient resource found in the bundle');
            return;
        }
    ...
    function getResourceByReference(bundle, reference) {
        ...
        const entry = bundle.entry.find(entry => entry.resource.id === id);
    ...
    function populateContent(bundle) {
        ...
        const sections = compositionEntry.resource.section;
